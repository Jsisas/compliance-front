describe('When visiting login page', () => {
	...
	it('google login should be visible and clickable', () => {
		cy.visit('/', {
			onBeforeLoad(win) {
				cy.stub(win, 'open');
			},
	...
	describe('when auth cookie is present', () => {
		before(() => {
		...
		it('it should redirect', () => {
			cy.visit('/');
			cy.url().should('eq', Cypress.config().baseUrl + '/regulations');
		});
	});

	describe('when auth cookie is not present', () => {
		it('it should redirect back to login page', () => {
